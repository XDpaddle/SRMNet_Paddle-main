    ...
    for net in net_l:
        for m in net.named_children():
            if isinstance(m, nn.Conv2D) or isinstance(m, nn.Conv3D):
                ...
                param_init.kaiming_normal_init(m.weight)
                m.weight *= scale  # for residual block
                if m.bias is not None:
                    param_init.constant_init(m.bias,value=0)
                ...
                param_init.constant_init(m.bias, 0.0)
...
def rotate(x, angle):
    if angle == 0:
        ...
        return paddle.rot90(x, k=1, axes=[3, 2])
    elif angle == 180:
        return paddle.rot90(x, k=2, axes=[3, 2])
    elif angle == 270:
        return paddle.rot90(x, k=3, axes=[3, 2])
...
class UNet(nn.Layer):
    ...
    def forward(self, x):
        ...
        if blindspot:
            x = paddle.concat([rotate(x, a) for a in [0, 90, 180, 270]], axis=0)
        # Encoder part
        pool0 = x
        ...
        if blindspot:
            ...
            x = pad(x[:, :, :-1, :])
            x = paddle.split(x, split_size_or_sections=x.shape[0] // 4, axis=0)
