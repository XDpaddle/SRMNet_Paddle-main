...
def mosaic(images):
  ...
  output = paddle.stack((red, green_red, green_blue, blue), axis=1)
  return output

class Mosaic_Operation(nn.Layer):
    ...
    def _initialize_weights(self):
        for m in self.named_children():
            if isinstance(m, nn.Conv2D):
                param_init.orthogonal_init(m.weight)
                print('init weight')
                if m.bias is not None:
                    param_init.constant_(m.bias, 0)
            elif isinstance(m, nn.BatchNorm2D):
                param_init.constant_init(m.weight, 1)
                param_init.constant_init(m.bias, 0)

class PACK2RGB(nn.Layer):
    def __init__(self, channels=3, filters_num = 128, filters_pack = 4):
...
class pseudoisp(nn.Layer):
    def __init__(self, channels=3, filters_num = 128, filters_pack = 3):
        super(pseudoisp, self).__init__()
